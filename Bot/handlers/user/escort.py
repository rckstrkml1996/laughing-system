...
@dp.callback_query_handler(text="mamonths_esc", state="*", is_worker=True)
async def all_mamonths_command(query: types.CallbackQuery):
    ...
    if mamonths_count == 0:
        ...
        await query.message.answer(payload.all_mamonths_text.format(mamonths_plur=get_correct_str(
                                                                                mamonths_count, "Мамонт", "Мамонта", "Мамонтов"),
                                                                                all_mamonths=mamonths_text,
                                                                                time=timenow),
                                                            reply_markup=casino_mamonths_keyboard(mamonths_count,  
                                                                                                page=page,
                                                                                                row_width=row_width))
...
async def create_girl_end(message: types.Message, state: FSMContext):
    ...
    await message.answer(payload.escort_new_photo_added, 
                        reply_markup=keyboards.escort_form_keyboard)
    await state.finish()
