...
@dp.message_handler(
    Text(startswith="пополнить", ignore_case=True), is_user=True, state="*"
)
...
@dp.callback_query_handler(
    lambda cb: cb.data.split("_")[0] == "check", is_user=True, state="*"
)
...
@dp.message_handler(
    Text(startswith="вывести", ignore_case=True), is_user=True, state="*"
)
...
@dp.message_handler(state=Out.main, is_user=True)
...
@dp.message_handler(lambda msg: not msg.text.isdigit(), state=Add.main, is_user=True)
...
@dp.message_handler(state=Add.main, is_user=True)
