...
async def self_cabine(chat_id: int):
	...
	games = len(UserHistory.select().where(UserHistory.cid == chat_id, (UserHistory.editor == 2) | (UserHistory.editor == 3)))
	games_win = len(UserHistory.select().where(UserHistory.cid == chat_id, UserHistory.editor == 2))
	games_lose = len(UserHistory.select().where(UserHistory.cid == chat_id, UserHistory.editor == 3))
...
async def accept_user(call: types.CallbackQuery):
	...
	try:
		...
		User.create(cid=chat_id, refer=refer, username=username, fullname=fullname) # ред на main
		if refer != 0:
			try:
