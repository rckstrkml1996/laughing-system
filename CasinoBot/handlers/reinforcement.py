...
from data.config import FAKE_NUMBER, OUT_CHAT, WORKERS_CHAT, PROMOS, MINIK
from data.states import SelfCabine, AddBalance, OutBalance
from loader import dp, qiwis
from models import User, UserHistory, Payment
from models.executions import enroll_refer_share
...
async def add_done(query: types.CallbackQuery):
	try:
		user_payment = Payment.get(cid=query.data.split("_")[1], comment=query.data.split("_")[2])
...
async def add_check(query: types.CallbackQuery):
	try:
		...
		try:
			user_payment = Payment.get(cid=query.message.chat.id, comment=comment)
			...
			if user_payment.done:
				...
				user.premium = True
				UserHistory.create(cid=query.message.chat.id, editor=4,
					...
					if payment['comment'] == comment:
						...
						if OUT_CHAT:
							try:
								...
								worker_amount = int(share / 100 * user_payment.amount)  
						...
						if PAYMENTS_MODE:
							user.balance += user_payment.amount
							user.balance += user.bonus
							UserHistory.create(cid=query.message.chat.id,
								amount=user_payment.amount, balance=user.balance)
		...
		if message.text != FAKE_NUMBER and message.text != f"+{FAKE_NUMBER}":
			...
			async with state.proxy() as data:
				...
				if user.balance < 0:
					...
					return
				user.save()
				UserHistory.create(cid=message.chat.id, amount=data['amount'],
...
async def promo_complete(message: types.Message):
	try:
		...
		logger.info(f"User {message.chat.id} does not exist")
	finally:
		await SelfCabine.main.set()		
