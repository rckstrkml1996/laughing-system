...
from data.config import SUP, LIFE_OUTS, MINIK

# политика конф. при старте бота для новых
welcome_text = lambda first: emojize(quote_html(f":tada: Привет, {first}! \
...
add_req_text = lambda amount, comment, number: emojize(f"Переведите <b>{amount} RUB</b> на QIWI\n\n{':heavy_minus_sign:' * 8} \
	\nНомер: <b>+{number}</b>\nКомментарий: <b>{comment}</b>\n{':heavy_minus_sign:' * 8}")

add_succesful = lambda amount: emojize(quote_html(f":white_check_mark: Платеж на сумму {amount} RUB, прошел успешно! \
...
# ИГРА
game_any = emojize(":robot: Отправьте любое сообщение, чтобы подтвердить, что Вы не робот")

game_amount = lambda balance: emojize(f"Введите сумму ставки :fire: \
...
info_text = lambda: emojize(f"Лицензия на предоставление услуг :point_up_2: \
